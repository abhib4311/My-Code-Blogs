...
export const getStaticProps = async (context) => {
  ...
  const page = allBlogs.find(
    (blog) =>
      String(blog.data.Title.split(" ").join("-").replace(/:/g, '-').toLowerCase()) === params.id
  );
  if (!page) {
    console.log("No page found for the given id:", params.id);
    return {
      notFound: true,
    };
  }
...
export const getStaticPaths = () => {
  ...
  return {
    paths: allBlogs.map((blog) => ({
      params: {
        id: String(blog.data.Title.split(" ").join("-").replace(/:/g, '-').toLowerCase()),
      },
    })),
    fallback: false,
  };
  ...
  return (
    <>
      <Head>
        ...
        <meta property="og:url" content="https://blogs.abhibhi.tech/" />
        <meta property="og:title" content={data.Title} />
        <meta property="og:description" content={data.Abstract} />
        <meta
          property="og:image"
          content={`https://raw.githubusercontent.com/abhibhi/Bits-0f-C0de/main/public${data.HeaderImage}`}
        />
